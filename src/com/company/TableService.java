...
public class TableService {
    ...
    Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/sakila?user=root&password=Password&serverTimezone=UTC");
    ...
    public String recordDelete(int actor_id, String first_name, String last_name) {
        try {
            stmt = conn.createStatement();
            stmt.executeUpdate("DELETE FROM actor WHERE actor_id=" + "'" + actor_id + "'" + " AND " + " first_name=" + "'" + first_name + "'" + " AND " + "last_name=" + "'" + last_name + "'");
            result = first_name + " " + last_name + " " + sdf3.format(currentDate);
    ...
    public String recordUpdate(Integer actor_id, String first_name, String last_name) {
        try {
            stmt = conn.createStatement();
            stmt.executeUpdate("UPDATE actor SET first_name=" + "'" + first_name + "'" + "," + "last_name=" + "'" + last_name + "'" + "," + "last_update=" + "'" + sdf3.format(currentDate) + "'" + " WHERE " + "actor_id=" + "'" + actor_id + "'");//???
            result = first_name + " " + last_name + " " + sdf3.format(currentDate);
